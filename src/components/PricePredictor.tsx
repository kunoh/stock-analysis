...
export function PricePredictor({ symbol, currentPrice, metrics, marketCap, currency }: Props) {
  ...
  // Get current multiples from metrics — null if unavailable (no synthetic defaults)
  const currentMultiples: Record<MultipleType, number | null> = {
    pe:        metrics.peRatio      ?? null,
    evEbit:    computedEvToEbit     ?? null,
    evEbitda:  computedEvToEbitda   ?? null,
    evRevenue: computedEvToRevenue  ?? null,
    evFcf:     computedEvToFcf      ?? null,
  };

  // Actual current margin for each method — null if data is missing.
  const rev = metrics.revenue ?? null;
  const currentMargins: Record<MultipleType, number | null> = {
    pe:        metrics.netMargin ?? null,
    evEbit:    rev && metrics.ebit        ? (metrics.ebit        / rev) * 100 : null,
    evEbitda:  rev && metrics.ebitda      ? (metrics.ebitda      / rev) * 100 : null,
    evRevenue: 0, // margin not used for EV/Revenue
    evFcf:     rev && metrics.freeCashFlow && metrics.freeCashFlow > 0
                 ? (metrics.freeCashFlow / rev) * 100
                 : null,
  };

  const methodAvailable = (type: MultipleType): boolean => {
    if (type === 'evRevenue') return currentMultiples.evRevenue !== null;
    return currentMultiples[type] !== null && currentMargins[type] !== null;
  };

  const marginLabels: Record<MultipleType, string> = {
    pe:        'Target Net Margin',
    evEbit:    'Target EBIT Margin',
    evEbitda:  'Target EBITDA Margin',
    evRevenue: 'Target Margin (unused)',
    evFcf:     'Target FCF Margin',
  };

  const initialMethod: MultipleType =
    (Object.keys(MULTIPLE_LABELS) as MultipleType[]).find(methodAvailable) ?? 'pe';
  const initialMargin = currentMargins[initialMethod] ?? 0;
  const initialMultiple = currentMultiples[initialMethod] ?? 0;
  const initialSpread = Math.max(Math.abs(initialMargin) * 0.25, 5);

  const [assumptions, setAssumptions] = useState<ProjectionAssumptions>({
    revenueGrowth: { bear: 5, base: 10, bull: 18 },
    targetMargin: { bear: initialMargin - initialSpread, base: initialMargin, bull: initialMargin + initialSpread },
    exitMultiple: { bear: initialMultiple * 0.7, base: initialMultiple, bull: initialMultiple * 1.3 },
    multipleType: initialMethod,
    dilutionRate: 1,
    years: 5,
  });
  ...
  const fairValueToday = useMemo(() => {
    ...
    const sharesOutstanding = metrics.sharesOutstanding ?? 0;
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="flex gap-2">
          {(Object.keys(MULTIPLE_LABELS) as MultipleType[]).map((type) => (
            <button
              key={type}
              disabled={!methodAvailable(type)}
              onClick={() => {
                const base = currentMargins[type];
                const multiple = currentMultiples[type];
                if (base === null || multiple === null) return;
                const spread = Math.max(Math.abs(base) * 0.25, 5);
                setAssumptions((prev) => ({
                  ...prev,
                  multipleType: type,
                  targetMargin: { bear: base - spread, base, bull: base + spread },
                  exitMultiple: {
                    bear: multiple * 0.7,
                    base: multiple,
                    bull: multiple * 1.3,
        ...
        <p className="text-sm text-gray-500 mt-2">
          Current {MULTIPLE_LABELS[assumptions.multipleType]}:{' '}
          {currentMultiples[assumptions.multipleType] != null
            ? `${currentMultiples[assumptions.multipleType]!.toFixed(1)}x`
            : 'N/A'}
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            />
            <InputField
              label={marginLabels[assumptions.multipleType]}
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200 p-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="p-4 bg-white rounded-lg border border-gray-200">
            ...
            <p className="text-2xl font-bold text-gray-900">{sym}{currentPrice.toFixed(2)}</p>
          </div>
          <div className="p-4 bg-white rounded-lg border-2 border-red-200">
            <p className="text-sm text-red-600">🐻 Bear Fair Value</p>
            <p className="text-2xl font-bold text-red-600">{sym}{fairValueToday.bear.toFixed(2)}</p>
          ...
          <div className="p-4 bg-white rounded-lg border-2 border-blue-300">
            ...
            <p className="text-2xl font-bold text-blue-600">{sym}{fairValueToday.base.toFixed(2)}</p>
          ...
          <div className="p-4 bg-white rounded-lg border-2 border-green-200">
            ...
            <p className="text-2xl font-bold text-green-600">{sym}{fairValueToday.bull.toFixed(2)}</p>
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={fullChartData} margin={{ top: 5, right: 30, bottom: 5, left: 10 }}>
              ...
              <YAxis
                tickFormatter={(val) => `${sym}${val.toFixed(0)}`}
                tick={{ fontSize: 12, fill: '#6b7280' }}
                width={70}
              />
              <Tooltip
                formatter={(value) => value !== undefined ? [`${sym}${Number(value).toFixed(2)}`, ''] : ['', '']}
          ...
          <table className="w-full">
            ...
            <tbody>
              <tr className="border-b border-gray-100 bg-blue-50">
                ...
                <td className="py-3 px-4 text-right text-sm font-medium text-red-600">{sym}{fairValueToday.bear.toFixed(2)}</td>
                <td className="py-3 px-4 text-right text-sm font-medium text-blue-600">{sym}{fairValueToday.base.toFixed(2)}</td>
                <td className="py-3 px-4 text-right text-sm font-medium text-green-600">{sym}{fairValueToday.bull.toFixed(2)}</td>
              </tr>
              {projections.map((p) => (
                <tr key={p.year} className="border-b border-gray-100">
                  <td className="py-3 px-4 text-sm text-gray-600">{p.year}</td>
                  <td className="py-3 px-4 text-right text-sm font-medium text-red-600">{sym}{p.bearCase.toFixed(2)}</td>
                  <td className="py-3 px-4 text-right text-sm font-medium text-gray-900">{sym}{p.baseCase.toFixed(2)}</td>
                  <td className="py-3 px-4 text-right text-sm font-medium text-green-600">{sym}{p.bullCase.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
