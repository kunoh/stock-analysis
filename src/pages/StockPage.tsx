...
import { SearchBar, PriceChart, ValuationMetrics, StockShareCard, FinancialOverview, PricePredictor, HistoricalFinancialsChart, StatCard } from '../components';
import { useStockPageData } from '../hooks/useStockPageData';
import { formatLargeNumber, formatCompactNumber, getCurrencySymbol } from '../utils/format';

type TabType = 'overview' | 'financials' | 'valuation' | 'predict' | 'share';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatCard label="52W High" value={`${sym}${quote?.high.toFixed(2)}`} />
              <StatCard label="52W Low" value={`${sym}${quote?.low.toFixed(2)}`} />
              <StatCard label="Volume" value={formatCompactNumber(quote?.volume)} />
              <StatCard label="Avg Volume" value="-" />
              <StatCard label="Open" value={`${sym}${quote?.open.toFixed(2)}`} />
              <StatCard label="Prev Close" value={`${sym}${quote?.previousClose.toFixed(2)}`} />
            </div>
        ...
        {activeTab === 'financials' && metrics && (
          <div className="space-y-6">
            <FinancialOverview metrics={metrics} currency={currency} />
        ...
        {activeTab === 'valuation' && metrics && (
          <ValuationMetrics metrics={metrics} price={quote?.price || 0} marketCap={quote?.marketCap || null} currency={currency} />
        )}

        {activeTab === 'share' && metrics && quote && (
          <StockShareCard
