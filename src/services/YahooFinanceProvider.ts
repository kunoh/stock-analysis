  ...
  async getFinancialMetrics(symbol: string): Promise<FinancialMetrics | null> {
    try {
      ...
      return {
        ...
        // freeCashflow from financialData is unreliable for non-US tickers.
        // Fall back to operatingCashflow + capitalExpenditures (capex is negative in Yahoo's data).
        freeCashFlow: (() => {
          const direct = num(fin?.freeCashflow);
          const opCf   = num(fin?.operatingCashflow);
          const capex  = num(fin?.capitalExpenditures); // negative value from Yahoo
          const computed = opCf != null && capex != null ? opCf + capex : null;
          // Prefer the direct field only if it looks plausible relative to operating cash flow.
          // If direct is more than 10× smaller than computed, it's likely bad data.
          if (direct != null && computed != null && Math.abs(direct) > Math.abs(computed) * 0.1) return direct;
